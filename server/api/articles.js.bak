...
module.exports = {
  ...
  'GET /onlyArticle': async (ctx, next) => {
    ...
    const doc = await db.article.find(params, (err, doc) => {
      if (err) {
        console.log(err)
      }
    })
    if (doc.length === 0) {
      ...
      ctx.rest(doc)
      // 更新文章的点击数
      db.article.update({ articleId: ctx.query.articleId }, { $inc: { pv: 1 } }, (err, doc) => {
        if (err) {
          console.log(err)
        }
      })
  ...
  'GET /getAdminArticles': async (ctx, next) => {
    ...
    try {
      const docs = await db.article.find(params, { content: 0 }).sort({ _id: -1 }).skip(skip).limit(limit)
      ctx.rest(docs)
    } catch (error) {
      console.log(error)
    }
  ...
  'GET /getAdminArticle': async (ctx, next) => {
    confirmToken(ctx, next)
    const doc = await db.article.find(ctx.query, (err, doc) => {})
    ctx.rest(doc)
  },
  // 存储文章
  'POST /saveArticle': async (ctx, next) => {
    ...
    const newDoc = await db.article.create(newArticle)
    if (newDoc._id) {
      ctx.rest({ code: 200 })
    }
  ...
  'DELETE /deleteArticle': async (ctx, next) => {
    ...
    const result = await db.article.remove({ articleId: { $in: ctx.query.articleId } })
    if (result.ok) {
